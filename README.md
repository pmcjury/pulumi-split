...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `split` with the `NAME` of your provider.
...
- binary: pulumi-resource-split
  ...
  ignore: []
  ldflags:
  - -X github.com/pulumi/pulumi-split/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-split/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-split_darwin_amd64_v1
│   └── pulumi-resource-split
├── pulumi-split_darwin_arm64
│   └── pulumi-resource-split
├── pulumi-split_linux_amd64_v1
│   └── pulumi-resource-split
├── pulumi-split_linux_arm64
│   └── pulumi-resource-split
├── pulumi-split_windows_amd64_v1
│   └── pulumi-resource-split.exe
└── pulumi-split_windows_arm64
    └── pulumi-resource-split.exe
```
